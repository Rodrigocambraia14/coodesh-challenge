...
import type { ApiResponseWithData, SaleResponse, UpdateSaleRequest } from './sales.api';
...
@Component({
  ...
  template: `
    <mat-card>
      <mat-card-title>Editar compra</mat-card-title>
      <mat-card-content>
...
export class SaleEditPage {
  ...
  /** Sincroniza rótulos dos lookups após carregar a venda (opções podem não incluir o id ainda). */
  private syncLookupDisplays(s: SaleResponse) {
    const cust: UserListRow = {
      id: s.customerId,
      username: s.customerDescription,
      email: '',
      role: 'Customer',
      status: 'Active'
    };
    this.customerFilterCtrl.setValue(cust, { emitEvent: false });

    const br: BranchListRow = { id: s.branchId, name: s.branchDescription };
    this.branchFilterCtrl.setValue(br, { emitEvent: false });
  }

  addItem(initial?: { productId?: string; productDescription?: string; quantity?: number; unitPrice?: number }) {
    this.items.push(
      this.fb.group({
        productId: [initial?.productId ?? '', [Validators.required]],
        productDescription: [initial?.productDescription ?? '', [Validators.required]],
        quantity: [initial?.quantity ?? 1, [Validators.required, Validators.min(1)]],
        unitPrice: [initial?.unitPrice ?? 1, [Validators.required, Validators.min(0)]]
      })
    );

    const idx = this.items.length - 1;
    const pid = initial?.productId;
    if (pid) this.onProductSelectChange(idx, pid);
  }
  ...
  load() {
    this.http.get<ApiResponseWithData<SaleResponse>>(`/api/Sales/${this.id}`).subscribe({
      next: (res) => {
        const s = res.data;
        if (!s) return;
        this.form.patchValue({
        ...
        this.syncLookupDisplays(s);
        this.items.clear();
        for (const it of s.items) this.addItem(it);
      },
      error: () => this.snack.open('Não foi possível carregar a compra.', 'OK', { duration: 2500 })
    });
  }
  ...
  submit() {
    ...
    this.http.put(`/api/Sales/${this.id}`, payload).subscribe({
      next: () => {
        this.snack.open('Compra atualizada.', 'OK', { duration: 2500 });
        this.router.navigate(['/sales', this.id]);
      },
      error: () => this.snack.open('Não foi possível atualizar a compra.', 'OK', { duration: 2500 })
    });
  }
}
